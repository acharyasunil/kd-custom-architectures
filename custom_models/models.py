...
from custom_models.model import Network
...
class CustomModels:
    ...
    def __init__(self, IN_CHANNEL, NUM_OUTPUT):
        ...
        self.model_25k_w_dw = nn.Sequential(
            nn.Conv2d(in_channels=self.input_channel, out_channels=8, kernel_size=(3, 3), padding=0, bias=False), # 32 -> 30
            nn.ReLU(),
            nn.BatchNorm2d(8),
            ...
            nn.Conv2d(in_channels=14, out_channels=14, kernel_size=(3, 3), padding=0, groups = 14, bias=False),  # 14 -> 12
            nn.Conv2d(in_channels=14, out_channels=24, kernel_size=(1, 1), padding=0, bias=False),
            nn.ReLU(),            
            nn.BatchNorm2d(24),


            nn.Conv2d(in_channels=24, out_channels=24, kernel_size=(3, 3), padding=0, groups = 24, bias=False),  # 12 -> 10
        ...
        ################################## 1 ST CONVOLUTIONAL BLOCK #####################################  
            nn.Conv2d(in_channels = self.input_channel, dilation  = 1, out_channels = 16, padding = 1, kernel_size= 3),                     # in 32, out 32, RF 3
            nn.ReLU(),
            nn.BatchNorm2d(16),
        ...
        self.model_1M_w_dw = nn.Sequential(
            ################################## 1 ST CONVOLUTIONAL BLOCK #####################################  
            nn.Conv2d(in_channels = self.input_channel, out_channels = 16, dilation  = 1, padding = 1, kernel_size= (3,3)),                     # in 32, out 32, RF 3
            nn.ReLU(),
    ...
    def init_model(self, model_name):
        if model_name == 'resnet_18':
            ...
            model = EfficientNet.from_pretrained('efficientnet-b5', in_channels=self.input_channel, num_classes=self.n_class_output)

        elif model_name == 'efficientnet-b7':
            model = EfficientNet.from_pretrained('efficientnet-b7', in_channels=self.input_channel, num_classes=self.n_class_output)
